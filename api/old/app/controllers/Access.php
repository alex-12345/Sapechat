...
class Access {
	 ...
	 * @param string password (get)
	 * http://localhost:8090/access-getTokenViaLogIn?email=vin-nikov@yandex.ru&password=d9c436b3d4e1f1d22614643588100e16
	 */
	public function getTokenViaLogIn(){
		...
		$user_id = $access->checkUserPswdAndGetUserId($_REQUEST['email'], md5($_REQUEST['password']));
		if($user_id > 0){
			$token = $access->createToken($user_id);
			echo json_encode(["error" => 0, "user_id" => $user_id, "token" => $token]);
		}elseif($user_id == 0){
			systemError(1, "Ошибка, неверный логин или пароль!");
		}else{
			systemError(2, "Некорректная передача параметров!");
		}
		unset($access);
	}
